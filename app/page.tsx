...
const Scene = dynamic(() => import('./house-scene'), { ssr: false });
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const updateUi = (progress: number) => {
      ...
      if (stageRef.current) stageRef.current.textContent = value < 0.3 ? '01 · OVERVIEW — 360° ORBIT' : value < 0.7 ? '02 · ASSEMBLY — EXPLODED VIEW' : '03 · COMPLETION — FINAL ADDRESS';
  ...
  return (
    <main className="min-h-screen overflow-x-clip bg-[#03070b] text-[#edf5f5]">
      ...
      <section id="overview" ref={heroRef} className="relative h-[300vh]"><div className="sticky top-0 h-screen overflow-hidden"><div className="relative h-full">{heroActive && <Scene active={heroActive} reduced={reduced} />}<div className="pointer-events-none absolute inset-0 z-10 bg-[radial-gradient(circle_at_50%_42%,transparent_0%,rgba(3,7,11,.1)_40%,rgba(3,7,11,.78)_100%)]" /><div className="pointer-events-none absolute inset-0 z-10 opacity-30 [background-image:linear-gradient(rgba(72,202,220,.06)_1px,transparent_1px),linear-gradient(90deg,rgba(72,202,220,.06)_1px,transparent_1px)] [background-size:72px_72px]" /><div className="relative z-20 flex h-full flex-col justify-center px-5 pt-16 sm:px-10">
