...
/* Legacy implementation retained during production migration.
...
export default function Home() {
  ...
  const showcaseVisual = facade === 'metal' || roof === 'green'
    ? {
        src: '/unsplash-white-house.jpg',
        credit: 'RETRATO INMOBILIARIO / Unsplash',
        href: 'https://unsplash.com/photos/modern-white-house-with-illuminated-pathway-at-dusk-eLG6MsOeupk',
      }
    : {
        src: '/unsplash-luxury-windows.jpg',
        credit: 'Michael Brown / Unsplash',
        href: 'https://unsplash.com/photos/modern-luxury-home-with-large-windows-at-dusk-G48h926L2qo',
      };
  const showcaseOverlay = facade === 'stone'
    ? 'linear-gradient(145deg, rgba(19, 25, 25, 0.32), rgba(145, 121, 85, 0.18))'
    : facade === 'metal'
      ? 'linear-gradient(145deg, rgba(12, 23, 33, 0.45), rgba(77, 112, 132, 0.2))'
      : 'linear-gradient(145deg, rgba(42, 19, 7, 0.2), rgba(3, 20, 26, 0.38))';
  ...
  return (
    <main className="min-h-screen overflow-x-clip bg-[#03070b] text-[#edf5f5]">
      ...
      <section id="showcase" className="border-t border-white/[0.08] bg-[#071117] px-5 py-28 sm:px-10 sm:py-40"><div className="mx-auto max-w-6xl"><div className="flex flex-col justify-between gap-10 md:flex-row md:items-end"><div><p className="text-[10px] uppercase tracking-[0.38em] text-cyan-200/80">Construction showcase</p><h2 className="mt-6 max-w-3xl text-5xl font-medium leading-[0.9] tracking-[-0.07em] sm:text-8xl">Tune the<br />composition.</h2></div><p className="max-w-xs text-sm leading-relaxed text-white/45">Choose the outer expression, the roof logic and the way the rooms open to the garden.</p></div><div className="mt-20 grid gap-10 lg:grid-cols-[1.1fr_0.9fr]"><div className="group relative min-h-[500px] overflow-hidden rounded-3xl border border-white/10 bg-[#0a1c24]"><AnimatePresence mode="wait"><motion.img key={showcaseVisual.src} initial={{ opacity: 0, scale: 1.035 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 1.018 }} transition={{ duration: 0.55, ease: [0.22, 1, 0.36, 1] }} src={showcaseVisual.src} alt="Modern house configuration preview" className="absolute inset-0 h-full w-full object-cover transition duration-700 group-hover:scale-[1.025]" /></AnimatePresence><div className="pointer-events-none absolute inset-0" style={{ background: showcaseOverlay }} /><div className="pointer-events-none absolute inset-0 bg-[linear-gradient(180deg,rgba(3,10,14,.2)_0%,rgba(3,10,14,.02)_32%,rgba(3,10,14,.86)_100%)]" /><div className="relative z-10 flex min-h-[500px] flex-col justify-between p-6 sm:p-8"><div className="flex items-center justify-between text-[10px] uppercase tracking-[0.25em] text-white/75"><span>House 01 / visual study</span><span className="font-mono text-cyan-100">LIVE</span></div><div className="self-start rounded-full border border-white/20 bg-[#061116]/65 px-3 py-2 text-[10px] uppercase tracking-[0.18em] text-white/80 backdrop-blur-md"><span className="text-cyan-100">{selectedFacade.label}</span><span className="mx-2 text-white/35">/</span>{selectedRoof.label}</div><div className="flex flex-col gap-5 border-t border-white/15 pt-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[10px] uppercase tracking-[0.24em] text-white/60">Estimated build</p><p className="mt-2 text-4xl tracking-[-0.06em] text-white">${price.toLocaleString()}</p><p className="mt-3 text-[10px] uppercase tracking-[0.22em] text-white/55">{selectedLayout.label} plan / 240 m²</p><a href={showcaseVisual.href} target="_blank" rel="noreferrer" className="mt-3 inline-block text-[9px] uppercase tracking-[0.16em] text-white/45 transition hover:text-cyan-100">Photo / {showcaseVisual.credit}</a></div><span className="w-fit rounded-full border border-cyan-100/45 bg-cyan-100/10 px-3 py-1.5 text-[10px] uppercase tracking-[0.2em] text-cyan-100 backdrop-blur-md">A+ / passive</span></div></div></div><div className="space-y-8"><Configurator title="Facade" options={facadeOptions} value={facade} onChange={setFacade} /><Configurator title="Roof material" options={roofOptions} value={roof} onChange={setRoof} /><Configurator title="Plan logic" options={layoutOptions} value={layout} onChange={setLayout} /></div></div></div></section>
